...
class BertDeepLncLocMultimodalExpression(Conv2dDeepLncLocMultimodalExpression):
    def __init__(self, train_seq=True, train_hl=True, n_heads=4, dropout_rate=0.1) -> None:
        super().__init__(n_heads=n_heads, dropout_rate=dropout_rate, mix_layer=True)
...
class BertDeepLncLocMultimodalExpressionTrainer(BaseTrainer):
    ...
    def generate_model_optimizer_criterion(self):
        model = BertDeepLncLocMultimodalExpression(train_seq=True, train_hl=True).to(torch.device("cuda"))
        # seq_pretrained_model = torch.load("./trained_models/saved/BertDeepLncLocMultimodalExpression-0.628-0.363.pt")
